...
// Creating cloth section content heading
var headingCloth = document.getElementById("cloth-heading");

// Heading to cloth section
headingCloth.innerHTML = "Clothing for Men and Women";

// Now comes to card view of clothing
var clothSecondDiv = document.getElementById("cloth-card-view");

// We req name, preview, isAccessory, brand, price


for (i = 0; i < productList.length; i++) {
  if (productList[i].isAccessory === false) {
    // We will create seprate articles for separate items
    var articleCloth = document.createElement("article");
    // Appending article to the second div of cloth
    clothSecondDiv.appendChild(articleCloth);

    // Firstly we require image of cloth to render
    var spanImg = document.createElement("span");
    // Append span to article
    articleCloth.appendChild(spanImg);

    // adding class to article cloth
    articleCloth.className = "cloth-article";

    // Adding preview to the SpanImg
    var clothImg = document.createElement("img");
    spanImg.appendChild(clothImg);
    clothImg.src = productList[i].preview;
    clothImg.className = "image-style-cloth";

    // Making content under card div
    var clothCardContent = document.createElement("div");

    // Appending div to article
    articleCloth.appendChild(clothCardContent);

    // Creating h3, h4, h5 under card 
    var cardHeading = document.createElement("h3");
    var cardBrand = document.createElement("h4");
    var cardPrice = document.createElement("h5");

    // Appending h3, h4, h5 to cloth card 
    clothCardContent.appendChild(cardHeading);
    clothCardContent.appendChild(cardBrand);
    clothCardContent.appendChild(cardPrice);

    // We will pass values to heading
    cardHeading.innerHTML = productList[i].name;

    // We will pass values to brand
    cardBrand.innerHTML = productList[i].brand;

    // We will pass values to Price
    cardPrice.innerHTML = "RS" + " " + productList[i].price;
  }
}

// End of Clothing Section .....


// Starting Accessories Section ....


// Creating Accessories section content heading
var headingAss = document.getElementById("accessories-heading");

// Heading to cloth section
headingAss.innerHTML = "Accessories for Men and Women";

// Now comes to card view of accessories
var assSecondDiv = document.getElementById("accessories-card-view");
...
for (i = 0; i < productList.length; i++) {
  if (productList[i].isAccessory === true) {
    // We will create seprate articles for separate items
    var articleAss = document.createElement("article");
    // Appending article to the second div of cloth
    assSecondDiv.appendChild(articleAss);

    // Firstly we require image of cloth to render
    var spanImg = document.createElement("span");
    // Append span to article
    articleAss.appendChild(spanImg);

    // adding class to article cloth
    articleAss.className = "ass-article";

    // Adding preview to the SpanImg
    var assImg = document.createElement("img");
    spanImg.appendChild(assImg);
    assImg.src = productList[i].preview;
    assImg.className = "image-style-ass";

    // Making content under card div
    var assCardContent = document.createElement("div");

    // Appending div to article
    articleAss.appendChild(assCardContent);
    ...
    // Appending h3, h4, h5 to cloth card 
    assCardContent.appendChild(cardHeading);
    assCardContent.appendChild(cardBrand);
    assCardContent.appendChild(cardPrice);
